...
class TransientMCPClient:
    ...
    async def _create_session(self, config, exit_stack):
        ...
        try:
            ...
            pxchat_logger.error(f"创建MCP会话失败: {e}")
            raise
        ...
        async with AsyncExitStack() as exit_stack:
            try:
                ...
                if not server_config:
                    pxchat_logger.info("没有启用的MCP服务器配置")
                    return []
                    
                for server_name, config in server_config.items():
                    try:
                        pxchat_logger.info(f"连接服务器 [{server_name}] 获取工具列表")
                        ...
                        for tool in response.tools:
                            ...
                            pxchat_logger.info(f"发现工具: {tool.name}")
                            
                    except Exception as e:
                        pxchat_logger.error(f"获取服务器 [{server_name}] 工具失败: {e}")
                        continue
                ...
                pxchat_logger.error(f"获取工具列表过程中发生错误: {e}")
                return []
    ...
    async def call_tool(self, tool_name: str, arguments: dict, timeout: float = 30.0):
        ...
        pxchat_logger.info(f"调用工具: {server_name}.{real_tool_name}, 参数: {arguments}")
        ...
        async with AsyncExitStack() as exit_stack:
            try:
                ...
                pxchat_logger.info(f"工具调用完成: {result[:100]}...")
                return result
                
            except asyncio.TimeoutError:
                pxchat_logger.warning(f"工具调用超时: {tool_name}")
                return f"工具调用超时，请稍后重试"
            except Exception as e:
                pxchat_logger.error(f"工具调用失败 {tool_name}: {e}")
