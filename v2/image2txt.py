...
async def recognize_image(
    ...
    try:
        pxchat_logger.info(f"开始识别图片: {image_url}")
        ...
        async with httpx.AsyncClient(
            ...
            pxchat_logger.info(
                f"图片下载状态码: {response.status_code}"
            )
        ...
        pxchat_logger.info(
            f"图片大小: {len(image_bytes) / 1024:.2f} KB"
        )
...
async def recognize_image_from_cache(
    ...
    try:
        pxchat_logger.info(f"开始识别缓存图片 ({len(image_bytes) / 1024:.2f} KB)")
        return await _do_recognize(image_bytes, prompt)
