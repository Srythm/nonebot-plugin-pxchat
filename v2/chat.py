...
async def get_chat_reply_with_tools(messages: list, is_group: bool = False) -> str:
    ...
    if not chat_manager.is_mcp_enabled():
        pxchat_logger.info("MCP功能未启用，使用普通聊天模式")
    ...
    try:
        ...
        if enabled_servers:
            try:
                ...
                pxchat_logger.info(f"MCP功能已启用，可用工具总数: {len(all_tools)} (本地: {len(local_tools)}, MCP: {len(mcp_tools)})")
            except Exception as e:
                pxchat_logger.warning(f"获取MCP工具失败，将只使用本地工具: {e}")
        else:
            pxchat_logger.info("没有启用的MCP服务器，只使用本地工具")
        ...
        if hasattr(response, 'usage') and response.usage:
            ...
            pxchat_logger.info(f"Function Call判断Token消耗 - 提示Token: {prompt_tokens}, 补全Token: {completion_tokens}, 总计: {total_tokens}")
        
        # 如果有工具调用，执行调用（在副本上进行）
        if tool_calls:
            pxchat_logger.info("检测到工具调用，开始执行函数")
        ...
        pxchat_logger.info(f"最近5条处理消息记录（含工具调用过程）:\n{tmp_record}")
        # 使用 processing_messages（包含工具调用过程）来生成最终回复
        pxchat_logger.info(f"调用原有逻辑生成格式化回复")
        ...
        pxchat_logger.error(f"get_chat_reply_with_tools 发生异常: {e}")
        # 如果工具调用失败，回退到普通聊天模式
        pxchat_logger.info("工具调用失败，回退到普通聊天模式")
        return await get_chat_reply(messages, is_group)
...
async def get_chat_reply(messages: list, is_group: bool = False) -> str:
    ...
    try:
        ...
        if hasattr(reply_obj, 'usage') and reply_obj.usage:
            ...
            completion_tokens = getattr(usage_info, 'completion_tokens', 0)
            total_tokens = getattr(usage_info, 'total_tokens', 0)
            pxchat_logger.info(f"对话Token消耗 - 提示Token: {prompt_tokens}, 补全Token: {completion_tokens}, 总计: {total_tokens}")
...
"""
        ...
        if hasattr(completion_obj, 'usage') and completion_obj.usage:
            ...
            completion_tokens = getattr(usage_info, 'completion_tokens', 0)
            total_tokens = getattr(usage_info, 'total_tokens', 0)
            pxchat_logger.info(f"判断Token消耗 - 提示Token: {prompt_tokens}, 补全Token: {completion_tokens}, 总计: {total_tokens}")

        pxchat_logger.info(f"群聊回复判断结果: {judgment.strip().upper()}")
