...
class ChatManager:
    ...
    def _load_manager_config(self):
        ...
        if os.path.exists(MANAGER_FILE):
            try:
                ...
                pxchat_logger.info("配置加载成功")
            except Exception as e:
                pxchat_logger.error(f"加载聊天管理器配置失败: {e}")
            ...
            pxchat_logger.info("创建新的聊天管理器配置")
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enabled_groups": [],
            "chat_enabled": True,
            "group_chat_probability": 1,  # 全局群活跃度基础值
            "group_probabilities": {},    # 每群独立活跃度配置 {group_id: probability}
    ...
    def _save_manager_config(self):
        ...
        try:
            ...
            pxchat_logger.error(f"保存聊天管理器配置失败: {e}")
    ...
    def get_group_probability(self, group_id: str) -> float:
        """获取指定群聊的触发概率，优先使用群独立配置，否则使用全局配置"""
        group_probs = self._data.get("group_probabilities", {})
        if group_id in group_probs:
            return group_probs[group_id]
        return self.get_group_chat_probability()
    
    def set_group_probability(self, group_id: str, probability: float) -> bool:
        """为指定群聊设置独立的触发概率"""
        if not 0 <= probability <= 1:
            return False
        
        if "group_probabilities" not in self._data:
            self._data["group_probabilities"] = {}
        
        self._data["group_probabilities"][group_id] = probability
        self._save_manager_config()
        return True
    
    def remove_group_probability(self, group_id: str) -> bool:
        """移除指定群聊的独立概率配置，恢复使用全局配置"""
        group_probs = self._data.get("group_probabilities", {})
        if group_id in group_probs:
            del group_probs[group_id]
            self._save_manager_config()
            return True
        return False
    
    def get_all_group_probabilities(self) -> Dict[str, float]:
        """获取所有群聊的独立概率配置"""
        return self._data.get("group_probabilities", {})
    ...
    def is_thinking_enabled(self) -> bool:
        """检查当前AI配置是否启用了思考模式"""
        config = self.get_current_ai_config()
        return config.get("thinking", False)
    
    def add_ai_config(self, name: str, api_key: str, api_url: str, model: str, thinking: bool = False) -> bool:
        """添加新的AI配置"""
