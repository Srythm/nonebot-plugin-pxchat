...
async def send_forward_message(user_id: int = None, group_id: int = None, messages: list = None):
    ...
    """
    if not messages:
        pxchat_logger.warning("合并转发消息内容为空")
        return None
    ...
    try:
        ...
        if not user_id and not group_id:
            pxchat_logger.error(f"参数错误: 至少指定 user_id 或 group_id 其中一个")
            return None
        ...
        pxchat_logger.info(f"合并转发消息发送成功")
        return result
        
    except Exception as e:
        pxchat_logger.error(f"发送合并转发消息失败: {e}")
        return None
...
async def send_error_to_super_users(error_msg: str, event: MessageEvent = None):
    ...
    if not super_users:
        pxchat_logger.warning("没有配置管理员，无法发送错误信息")
        return
    ...
    for user_id in super_users:
        try:
            ...
            pxchat_logger.info(f"已发送错误信息给管理员 {user_id}")
        except Exception as e:
            pxchat_logger.error(f"发送错误信息给管理员 {user_id} 失败: {e}")
            try:
                fallback_msg = f"聊天插件错误:\n{error_summary[:100]}..."
                await bot.send_private_msg(user_id=int(user_id), message=fallback_msg)
            except Exception as fallback_e:
                pxchat_logger.error(f"备用消息发送失败: {fallback_e}")
