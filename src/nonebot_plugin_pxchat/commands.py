...
🔧 AI配置管理
• px ai - 查看AI配置
• px ai add <名称> <key> <url> <模型> [thinking]
• px ai del <名称> - 删除配置
...
async def handle_ai_config(event: MessageEvent, args: Message = CommandArg()):
    ...
    if not arg_text:
        ...
        for i, config in enumerate(configs):
            ...
            thinking_tag = " 🧠" if config.get("thinking", False) else ""
            safe_key = config['api_key'][:6] + '***' if len(config['api_key']) > 6 else '***'
            content = f"{config['name']}{is_current}{thinking_tag}\n接口: {config['api_url']}\n模型: {config['model']}\n密钥: {safe_key}\n思考模式: {'开启' if config.get('thinking', False) else '关闭'}"
    ...
    if action == "add" and len(parts) >= 5:
        ...
        thinking = len(parts) >= 6 and parts[5].lower() in ("1", "true", "yes", "on")
        
        if chat_manager.add_ai_config(name, api_key, api_url, model, thinking):
            thinking_info = "（思考模式开启 🧠）" if thinking else ""
            await ai_cmd.finish(f"✅ 已添加配置: {name} {thinking_info}")
        else:
        ...
        await ai_cmd.finish("用法:\n• px ai - 查看配置\n• px ai add <名称> <key> <url> <模型> [1=开启思考]\n• px ai del <名称>\n• px ai switch <名称>")
